...
import { setupTestDB } from "../db/setupDb.test";
...
test("update post success", async () => {
  ...
  const saved = await AppDataSource.getRepository(Post).findOne({ where: { id: postId } as any });
  expect((saved as any)?.title).toBe("New");
});
